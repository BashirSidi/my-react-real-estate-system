...
function syncPluginVersionWithNextVersion() {
    ...
    if (!nextVersion) {
        utils_1.logger.error('[next-plugin-sentry] Cannot read next.js version. Plug-in will not work.');
        return;
    }
    ...
    if (!pluginPackageDotJson) {
        utils_1.logger.error("[next-plugin-sentry] Cannot read " + pluginPackageDotJsonPath + ". Plug-in will not work.");
        return;
    }
...
function withSentryConfig(providedExports, providedWebpackPluginOptions) {
    ...
    return {
        ...
        plugins: tslib_1.__spread((providedExports.plugins || []), ['@sentry/next-plugin-sentry']),
        ...
        webpack: function (config, _a) {
            ...
            new SentryWebpackPlugin(tslib_1.__assign(tslib_1.__assign({ dryRun: dev }, defaultWebpackPluginOptions), providedWebpackPluginOptions)));
            return config;
        },
    };
}
