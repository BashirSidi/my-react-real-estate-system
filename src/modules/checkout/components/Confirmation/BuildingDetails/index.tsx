...
const BuildingDetails: FC<IProps> = ({ booking }) => {
  ...
  const isBoxToBox = site_name === BOX_TO_BOX;
    ...
    <Box className={classes.buildingDetails}>
      <Box className={classes.header}>
        <img
          ...
          src={isBoxToBox ? original_space?.images?.[0] : original_site?.images?.[0]}
          alt={site_name}
        />
        <Box>
          {!isBoxToBox && (
            <Typography className={classes.location}>
              ...
              />
              <span>
                {getTranslatedName(site_address?.district, 'name', locale)}
              </span>
            </Typography>
          )}
          <Typography className={classes.title}>
            {site_name?.includes(BOX_TO_BOX) && isBoxToBox
              ? 'Box by Box'
              : site_name}
          </Typography>
        </Box>
        ...
        <Box className={classes.calendarBlock}>
          ...
          <Typography>
            <span>
              {auto_renewal
                ? t('span')
                : dayJs(move_out_date).format('DD MMM YYYY')}
            </span>
            <Image folder="BookingConfirmed" name="calendar" />
          </Typography>
        </Box>
        ...
        <Box className={classes.infoBlock}>
          ...
          <Box className={classes.infoBlockData}>
            ...
            <Typography variant="caption">
              {`${dayJs(pickup.order_pick_up_service.pickup_time).format(
                'h:mm A',
              )} - `}
              {dayJs(pickup.order_pick_up_service.pickup_time)
                .add(1, 'hour')
                .format('h:mm A')}
            </Typography>
          </Box>
          ...
          <Box className={classes.storage}>
            ...
            {!isBoxToBox && (
              <Box>
                <Typography className={classes.blockTitle}>
                  {original_space.size}
                  {`${original_space.size_unit}(${original_space.name})`}
                </Typography>
                <Typography className={classes.blockTitle}>
      ...
      {is_insured && !!insurance && (
        <Box className={classes.infoBlock}>
          ...
          <Box className={classes.flex}>
            ...
            <Box className={classes.infoBlockData}>
              <Typography>
                {`${getTranslatedName(insurance, 'name', locale)} ${t(
                  'typography7',
                )} ${insurance?.country?.currency_sign}${
              ...
              <Typography>
                {`${insurance?.country?.currency_sign}${
                  ...
                  && (insurance.price_per_day * 30).toFixed(2)
                } ${t('typography8')}`}
              </Typography>
            </Box>
          </Box>
