...
const BookingDetails: FC<IProps> = ({ booking, loading }) => {
  ...
  const {
    ...
    sub_total_amount, total_amount, status, renewals, unit_id, base_amount,
  } = booking || {};
    ...
    <Box className={classes.buildingDetails}>
      <Box className={classes.header}>
        <Box>
          ...
          {isBoxToBox && (
            <Typography className={classes.parts}>
              {boxFullName}
              {' '}
              (
              {boxCount}
        ...
        {isBoxToBox
          && (<img className={classes.img} src={original_space?.images?.[0]} alt={site_name} />
          )}
        {!isBoxToBox
      ...
      <Box className={classes.partWithTitle}>
        ...
        <Box>{orders?.[0]?.order_pick_up_service?.address}</Box>
        <Box display="flex" justifyContent="space-between" alignItems="baseline">
          <Box mt="9px">{t('typography4')}</Box>
          {orders.length > 0 && (
            <Box>
              {`${dayJs(orders?.[0]?.order_pick_up_service?.pickup_time).format(
                'h:mm A',
              )} - `}
              {dayJs(orders?.[0]?.order_pick_up_service?.pickup_time)
                .add(1, 'hour')
                .format('h:mm A')}
            </Box>
          )}
      ...
      && (BookingStatus.ACTIVE || BookingStatus.CONFIRMED || BookingStatus.TERMINATED) && (
        <Box
          display="flex"
          alignItems="center"
          className={classes.partWithTitle}
        >
          ...
          {transactions?.map((item) => (
            <Box className={classes.transaction} key={item.id}>
              ...
              <Grid md={2} xs={4}>
                <Box
                  display="flex"
                  paddingLeft={!isMobile ? '65px' : '16px'}
                >
