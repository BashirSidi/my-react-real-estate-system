...
const FormPaymentMethod: React.FC<IProps> = (props) => {
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      let order;
      let updatedBooking;
      if (bookingStore.serviceId && !bookingStore?.booking?.orders?.[0]?.order_pick_up_service) {
      ...
      if ((bookingStore.serviceId
        && !bookingStore?.booking?.orders?.[0]?.order_pick_up_service && !order)
        || (bookingStore.insuranceId && !updatedBooking)) {
        return;
      }
