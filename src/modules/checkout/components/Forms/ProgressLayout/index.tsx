...
const ProgressLayout: React.FC<IProps> = (props) => {
  ...
  const requirePickup = isBoxToBox && bookingStore?.serviceId < 1;
  const showButtons = () => {
      ...
      return bookingStore.currentStep === 1 ? (
        <Box display="flex" alignItems="center">
          <Buttons
            ...
            disableBtn={disableBtn || requirePickup}
            icon={btnIcon}
            btnText={btnText}
          ...
          />
          {!hideServices && bookingStore.serviceId !== 0 && !isBoxToBox && (
            <Typography
              onClick={() => {
          ...
          {requirePickup && (
            <Typography
              className={clsx(classes.noServices, classes.requiredPickUp)}
            >
              * Pick up service is required
            </Typography>
          )}
        </Box>
      ) : (
        ...
        <Box>
          <Box display="flex" justifyContent="space-between">
            <Box className={classes.headerBox}>
              ...
              {(step === bookingStore.currentStep && isShowSubtitle) && (
