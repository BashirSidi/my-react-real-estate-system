...
import { ITrackingPayloadBooking } from 'shared/interfaces';
...
import { removeLocalStorage } from '../../../utilities/localStorage';
import { GetPublicPromotionsQuery_promotions_edges } from '../queries/__generated__/GetPublicPromotionsQuery';
...
export const BOX_TO_BOX_ADDRESS_KEY = 'boxToBoxPickUpAddress';
...
export class BookingStore {
  ...
  @observable promoCode:string = null;

  @observable promoPublic:string = null;

  @observable currentStep = 0;

  @observable openPromoPopup = false;

  @observable promotion = false;
  ...
  @observable publicPromos: GetPublicPromotionsQuery_promotions_edges[];
  ...
  checkAvailability = async (): Promise<void> => {
    ...
    const spaces = data?.sites[0]?.spaces || [];
    ...
    this.apollo.query<GetBookingDetailsQuery, GetBookingDetailsQueryVariables>({
      ...
      runInAction(() => {
        ...
        this.bookingDetails = {
          ...
          moveOutDate: result?.data?.booking?.move_out_date
            ? DayJS(result?.data?.booking?.move_out_date)
            : undefined,
  ...
  createOrder = async (): Promise<FetchResult> => {
    const date = this.bookingDetails.moveInDate.format('YYYY-MM-DD');
    const time = `${checkTime(this.pickUpDetails.time.value)}:00`;
    const utcTime = DayJS(`${date} ${time}`).utc().format();
    const order = {
      time: utcTime,
  ...
  createBooking = async (): Promise<CreateBooking> => {
    ...
    const promotionId = details?.promoId;
    ...
    // check if there's any public promotion on a site
    if (this.publicPromos.length !== 0) {
      // check if public promotion id is a valid number then
      // we confirm the promotion is true otherwise system display
      // popup to confirm with a user
      // eslint-disable-next-line no-restricted-globals
      if (isNaN(promotionId) === true) {
        this.setPromotion(true);
      }

      // check if there's no promotion then we display popup
      // to let the user confirm if he still didn't want the
      // promotion
      if (this.promotion === false) {
        this.setOpenPromoPopup(true);
        return null;
      }
    }
  ...
  payBooking = (token: string, quotationId?: string): void => {
    ...
    this.apollo.mutate<PayBooking, PayBookingVariables>({
      ...
      try {
        const trackingPayload: ITrackingPayloadBooking = {
          bookingId: (this?.bookingId ?? 0).toString(),
          bookingStatus: 'CONFIRMED',
          spaceId: (this?.booking?.original_space.id).toString(),
          spaceSize: (this?.booking?.space_size).toString(),
          promoPublic: this.promoPublic,
          promoCode: this.promoCode,
          siteName: this?.booking?.site_name,
          siteId: (this?.booking?.original_site?.id || 0).toString(),
          insuranceName: this?.booking?.insurance?.name_en || '',
          insuranceAmount: (this?.booking?.renewals?.[0]?.insurance_amount || 0).toString(),
          currency: this?.booking?.currency ?? '',
          userId: (this?.booking?.customer?.id ?? 0).toString(),
          ...
          baseAmount: (this?.booking?.base_amount ?? 0).toString(),
          subTotalAmount: (this?.booking?.sub_total_amount ?? 0).toString(),
          depositAmount: (this?.booking?.deposited_amount ?? 0).toString(),
          totalAmount: (this?.booking?.total_amount ?? 0).toString(),
          discountAmount: (this?.booking?.discount_amount ?? 0).toString(),
        ...
        const dynamicAdsParams: IDynamicAdsParam = {
          ...
          content_ids: `${this?.booking?.original_site?.id ?? 0}`,
          currency: getCountry(this.defaultLocale).currency,
          availability: 'for_rent',
          country: getCountry(this.defaultLocale).name,
        };

        intercom.track(IEventName.CHARGED, trackingPayload);
        gtag.track('Purchase', dynamicAdsParams);
        branch.track(IEventName.CHARGED, trackingPayload);
        ClevertapReact.event(IEventName.CHARGED, trackingPayload);
        Router.push({
          pathname: '/checkout/confirmation',
          query: { booking_id: this.bookingId, isBoxToBox: true },
        });
      } catch (errEvent) {
        logErrorCleverTap(IEventName.CHARGED, errEvent);
      }
    }).catch((e) => {
      runInAction(() => {
        this.paymentError = e.message;
  ...
  // handling promo opening and closing dialog
  @action
  setOpenPromoPopup = (openPromoPopup: boolean): void => {
    this.openPromoPopup = openPromoPopup;
  };

  // handling continue without promotion button
  @action
  ...
  @action
  setPublicPromos = (
    publicPromos: GetPublicPromotionsQuery_promotions_edges[],
  ): void => {
    this.publicPromos = publicPromos;
  };
  ...
  selectService = (id: number, service: BookingServicesQuery_services_edges): void => {
    ...
    try {
      ...
      intercom.track(IEventName.SET_SERVICE, trackingPayload);
      ClevertapReact.event(IEventName.SET_SERVICE, trackingPayload);
    } catch (errEvent) {
      logErrorCleverTap(IEventName.SET_SERVICE, errEvent);
    }
  };
  ...
  setInsurancePlan = (id: number, insurance: BookingInsurancesQuery_insurances_edges): void => {
    ...
    if (this.booking) {
      try {
        ...
        intercom.track(IEventName.SET_INSURANCE, trackingPayload);
        ClevertapReact.event(IEventName.SET_INSURANCE, trackingPayload);
      } catch (errEvent) {
        logErrorCleverTap(IEventName.SET_INSURANCE, errEvent);
      }
    }
  };
}
