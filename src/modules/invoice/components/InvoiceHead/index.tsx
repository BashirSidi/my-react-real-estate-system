...
import { getCountryShortCodeByCurrency } from 'utilities/market';
...
const InvoiceHead: React.FC<IProps> = ({
  id, issue_date, due_date, items, booking_short_id,
}) => {
  const classes = useStyles();
  const countryShortCode = getCountryShortCodeByCurrency(items?.[0]?.currency);
      ...
      <Grid className={`${classes.spaceBetween} ${classes.mobileColumn}`} item>
        ...
        <Box className={classes.dateWidth} mt={10}>
          <Typography className={classes.spaceBetween} variant="body1">
            ...
            :
            &nbsp;&nbsp;
            <span className={classes.lightBold}>{booking_short_id}</span>
          </Typography>
          <Typography className={classes.spaceBetween} variant="body1">
            {t('typography7')}
            &nbsp;&nbsp;
            <span className={classes.lightBold}>{issue_date && format(new Date(issue_date), 'dd/MM/yyyy')}</span>
          </Typography>
          <Typography className={classes.spaceBetween} variant="body1">
            {t('typography8')}
            &nbsp;&nbsp;
            <span className={classes.lightBold}>{due_date && format(new Date(due_date), 'dd/MM/yyyy')}</span>
          </Typography>
          <Typography className={classes.spaceBetween} variant="body1">
