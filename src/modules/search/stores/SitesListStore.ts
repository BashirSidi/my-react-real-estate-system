...
export class SitesListStore {
  ...
  getDistricts = async (args: {
    countryId: number; moveInDate: string,
    withoutAvailabilityFilter?: boolean
  }) => {
    this.isLoading = true;
    const { countryId, moveInDate, withoutAvailabilityFilter } = args;
    const locations = countries.filter((country) => country.id === countryId);
    let sites = [];
    const districts: ILocalizedNames[] = [];
    try {
      if (!withoutAvailabilityFilter) {
        ...
        sites = res.data.hits.hits.filter((site) => {
        ...
        locations.forEach((location) => {
          location.cities.forEach((city) => {
            // NOTE:Temporary fix for Japan, only get Tokyo districts without filtering from stock
            if (city.id === 140) {
              city.districts.forEach((district) => {
                districts.push(district);
              });
  ...
  fetchSiteMarkers = async (viewMap = false) => {
    ...
    this.markers = [];
    this.mapOffset = 0;
    const canSearchByLatLng = !(this.cityId || this.districtId);
    ...
    if (this.searchedLatLon?.lat && this.searchedLatLon?.lon && canSearchByLatLng) {
      ...
      viewMapArgs.districtIds = this.districtFilterIds;
      viewMapArgs.cityIds = this.cityFilterIds;
    }
    const onMapArgs = {
