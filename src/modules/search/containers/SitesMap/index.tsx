...
const SitesMap: React.FC<IProps> = ({
  sitesStore, showMap, setShowMap, auth,
}) => {
  ...
  const {
    isLoadingForMap, mapSites, setCountryId, filters,
  } = sitesStore;
  ...
  useEffect(() => {
    ...
    if (sitesStore.mapRef && !sitesStore.sites.length) {
      setHasCalledES(true);
    }
  }, [sitesStore.markers]);
  ...
  const sendEvents = () => {
    setShowMap(true);
    const eventName = IEventName.MAP_VIEWED;
    try {
      ...
      logErrorCleverTap(eventName, e);
    }
  };

  useEffect(() => sendEvents(), []);
  ...
  const goToSiteDetails = (siteDetails: ISiteDetails) => {
    ...
    const url = `/details/${id}`.concat(`?version=${version}`);
    if (isMobile && id) {
      router.push(url);
      return;
    }
    if (id) {
      const newUrl = `/${router.locale}${url}`;
      window.open(newUrl);
    }
  };
...
export default inject(SITES_STORE_KEY, AUTH_STORE_KEY)(observer(SitesMap));
