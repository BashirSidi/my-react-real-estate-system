...
const SiteItem: React.FC<Props> = ({
  ...
  const trackingPlayLoad = {
    ...
    spaceSize: spaceType,
    amenitiesSelected: spaceFeatures,
  };

  const goToSiteDetails = (spaceId: number = null) => {
    try {
      ClevertapReact.event(IEventName.SITE_SELECTED_MAP, trackingPlayLoad);
      gtag.enhancedTrack({
        event: 'productClick',
        ecommerce: {
          click: {
      ...
      if (isMobile) {
        router.push({
          pathname: `${locale}/details/${id}`.concat(`?version=${version}`),
          query: {
            ...router.query,
            space_id: spaceId,
          },
    ...
    <div ref={ref} style={{ padding: '10px 0' }}>
      {isFeatured && isMobile && (
        <div className={classes.top}>
          <Icon name={`topPics${getLocalizedTopPicks()}`} folder="SearchLocation" />
        </div>
      )}
