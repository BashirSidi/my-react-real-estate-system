...
const SelectPlace: React.FC<IProps> = ({ sitesStore }) => {
  ...
  const countryId = +router?.query?.country_id || currentCountry?.id;
  const countryName = router?.query?.country_name || currentCountry?.name;
  let breadCrumbs: any[] = [];
  ...
  // NOTE: Temporary fix for Japan, search by districts and not cities
  const isJapan = countryName?.includes('Japan');
  // const isJapan = false;
  useEffect(() => {
    if (isJapan) {
      getCities({
      ...
      getDistricts({
        countryId,
        moveInDate,
        withoutAvailabilityFilter: !!countryName?.includes('Japan'),
      });
    }
  }, []);
  ...
  const searchStorage = () => {
    ...
    try {
      ...
      ClevertapReact.event(IEventName.SEARCH, trackingPayload);
      intercom.track(IEventName.SEARCH, trackingPayload);
      branch.track(IEventName.SEARCH, trackingPayload);
    } catch (errEvent) {
      logErrorCleverTap(IEventName.SEARCH, errEvent);
    }
      ...
      <Box className={classes.root} fontSize={12}>
        ...
        {isLoading ? <Skeleton width={450} className={classes.skeleton} variant="text" animation="wave" /> : (
          <Box className={classes.container}>
            {showAllOptions()}
            {isJapan ? showPrefectures() : showDistricts()}
