...
import { getTranslatedSizeUnit, useCurrentCountry } from 'utilities/market';
...
const SpaceItem: SpaceItemType = ({
  size, size_unit, prices, features, onClick, width, height, length,
}) => {
      ...
      <Box className={classes.boxBeforePrice}>
        <Box>
          <Typography variant="h4">
            {size}
            &nbsp;
            {getTranslatedSizeUnit(size_unit, locale)}
            <Dimensions
              width={width}
              height={height}
              length={length}
      ...
      <Box>
        <Typography variant="h2" className={classes.price}>
          ...
          {new Intl.NumberFormat(defaultLocale)
            .format(prices[0]?.price_per_month)}
        </Typography>
      </Box>
    </Box>
  );
