...
const FeaturedListing: FC<IProps> = ({ site, index, auth }) => {
          ...
          <Box className={classes.price}>
            <Typography variant="h3">
              ...
              {new Intl.NumberFormat(defaultLocale)
                .format(site?.spaces?.edges?.[0]?.prices?.[0]?.price_per_month)}
              /
            </Typography>
            <Typography variant="body2">
              {' '}
...
export default inject(AUTH_STORE_KEY)(observer(FeaturedListing));
