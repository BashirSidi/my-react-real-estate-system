...
const SearchAutoComplete: React.FC<IProps> = ({
  ...
  const { locations } = esStore;
  const isJapan = currentCountry.name === FixedCountry.Japan;
  const showCities = () => {
    if (address.length < 1) {
    ...
    setSearchBtn(true);
    setDbSuggestions(
      searchTerm({ country: country.name, showCities: true, locations }, t).slice(0, 5),
    );
  };

  const handleChange = (val: string) => {
    setShowDefaultSuggest(true);
    const searchTerms = searchTerm({ term: val, country: country.name, locations }, t).slice(0, 3);
    setDbSuggestions(searchTerms);
    setAddress(val);
    if (val?.length <= 0) showCities();
  };
  ...
  */
  ...
  const handleDbSelect = (val: ILocation, e) => {
    ...
    try {
      ...
      const eventName = IEventName.SEARCH;
      ...
      {showDefaultSuggest && (!!address.length || showDefaultCities) && !!dbSuggestions.length && (
        ...
        >
          {(!!address.length || showDefaultCities)
            && dbSuggestions.map((s, i) => !!s && renderAutocompleteList(s, i))}
        </div>
      )}
    </>
  );
...
export default inject(SITES_STORE_KEY, STORE_KEY)(observer(SearchAutoComplete));
