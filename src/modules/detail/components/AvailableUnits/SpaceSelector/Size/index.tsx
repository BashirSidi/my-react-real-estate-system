...
import { getTranslatedName, getTranslatedSizeUnit, useCurrentCountry } from 'utilities/market';
    ...
    >
      {space.stock_available_until && (
        <Box className={classes.stockDate}>
          ...
          {t('availability')}
          &nbsp;
          <span>{DayJS(space.stock_available_until).locale(locale).format('DD MMMM YYYY')}</span>
        </Box>
      )}
      ...
      <Box className={classes.boxBeforePrice}>
        ...
        <Box>
          <Typography variant="caption" className={classes.variantDescription}>
            {space.features.map((f) => getTranslatedName(f, 'name', locale)).slice(0, 3).join(', ')}
          </Typography>
        </Box>
      </Box>
      <Box>
