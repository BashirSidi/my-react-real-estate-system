  ...
  return (
    <Box className={classes.root}>
      ...
      <Box className={classes.content} display="flex" justifyContent={showPromotions ? 'space-between' : 'center'} flexDirection={isMobile ? 'column' : 'row'}>
        <SelectMoveInDate showPromotions={showPromotions} onChange={(date) => setQuotationDetails('moveInDate', date)} value={moveInDate} minValue={DayJS().add(1, 'day')} maxValue={DayJS().add(29, 'day')} />
