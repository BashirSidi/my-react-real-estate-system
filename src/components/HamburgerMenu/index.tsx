...
import { ITrackingSignOut } from '../../shared/interfaces';
...
const HamburgerMenu: React.FC<IProps> = (props) => {
  ...
  const trackingSignOut: ITrackingSignOut = {
    platform: 'WEB',
    language: lang,
    identity: '',
  ...
  const triggerSignOutEvent = () => {
    try {
      const eventName = IEventName.SIGN_OUT;
      gtag.track(eventName, trackingSignOut);
      intercom.track(eventName, trackingSignOut);
      ClevertapReact.event(eventName, trackingSignOut);
    } catch (errEvent) {
      logErrorCleverTap(IEventName.SIGN_OUT, errEvent);
    }
  };
