...
interface IProps {
  isOpen: boolean;
  handleVirtualClick: () => void;
  onClose(): void;
  images: string[];
  initialIndex?: number;
  showVirtualViewIcon: boolean;
}
...
const CarouselPopup: React.FC<IProps> = ({
  isOpen, onClose, images, initialIndex, handleVirtualClick, showVirtualViewIcon,
}) => {
  ...
  return (
    <Dialog fullScreen open={isOpen} onClose={onClose} classes={{ paper: classes.paper }}>
      ...
      {showVirtualViewIcon && (
      <Box className={classes.virtualWrapper}>
        <Box className={classes.virtualContainer}>
          <Image folder="DetailPage" name="virtualView" className={classes.virtual} onClick={handleVirtualClick} />
        </Box>
      </Box>
      )}
      {renderKeenSlider()}
