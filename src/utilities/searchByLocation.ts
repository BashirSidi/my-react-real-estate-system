import { TOKYO_CITY_ID } from 'config';
...
const searchTerm = (searchQueries: ISearchTerm, t = null): ILocation[] => {
  ...
  const {
    term = '', country, showCities, locations,
  } = searchQueries;
  const countryData = locations.find((data) => data.name_en === country);
  if (!countryData) {
    return result;
  }
  let flatternedLocations: ILocation[] = [];
  ...
  const TOKYO = citiesOnTop.filter((location) => location.city.id === TOKYO_CITY_ID)[0];
  const isJapan = country === FixedCountry.Japan;
  if (!term && !isJapan) return [cityOnTop];
  const places = {
    places: [{
  ...
  // NOTE: Temporary fix for Japan, hide cities, only show tokyo and its districts
  if (!showCities && isJapan) {
    // eslint-disable-next-line
    flatternedLocations = flatternedLocations.filter((location) => location.city.id === TOKYO_CITY_ID);
  }
  // if request show all
  if (((term && term.length < 2) || !term) && isJapan) {
    if (showCities) {
      citiesOnTop.unshift(places);
      return citiesOnTop.slice(0, 6);
    }

    return [places, TOKYO, ...flatternedLocations].slice(0, 6);
  }
