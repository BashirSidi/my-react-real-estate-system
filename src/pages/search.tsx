...
import HowItWorks from 'modules/search/components/HowItWorks';
import Faqs from 'modules/search/components/Faqs';
...
const SizeEstimatorPlayer = dynamic(() => import('../modules/search/components/SizeEstimatorPlayer'));
...
const Search: React.FunctionComponent<IProps> = ({ sitesStore, auth }) => {
  ...
  const totalResults = sitesStore.total || 0;
  ...
  useEffect(() => {
    ...
    if (filterBy) {
      ...
      if (filterBy === 'districts') {
        ...
        ids = JSON.parse(cityIds as string);
        sitesStore.setCityFilterIds(ids[0] !== 0 ? ids : []);
      }
    }
    if (decodeURI(params.address)) {
      if (typeof window !== 'undefined') {
        window.localStorage.removeItem('searchQueryParams');
                  ...
                  <Hidden smDown>
                    <Box style={{ marginLeft: '15px' }}>
                      <Typography variant="h1">
                        {t('h1', { location: address?.length ? address : getTranslatedName(country, 'name', router.locale) }) }
                      </Typography>
...
export default inject(SITES_STORE_KEY, AUTH_STORE_KEY)(observer(Search));
